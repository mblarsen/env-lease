...
`, destinationOutside)
	...
	if !strings.Contains(stderr, expectedError) {
		t.Errorf("Expected error message to contain '%s', but it was: %s", expectedError, stderr)
	}

	// Test case 2: Destination outside root, with override flag (should succeed)
	_, stderr, err = runGrantCommand(t, "--config", configFile, "--destination-outside-root")
	if err != nil {
